...
const RewiewsMessage = ({ events, data }) => {
    const { commentsCount, retweetCount, likesNumState, activeLikeClass } = data;
    const { comments, retweets, likes } = events;

    return (
        <div className='message-rewiews'>
            <button className='comments' onClick={comments?.onClick}>
            ...
            <button className='retweets' onClick={retweets?.onClick}>
                <svg width="22" height="22" viewBox="0 0 22 22" fill='none' xmlns="http://www.w3.org/2000/svg">
            ...
            <button className={`likes ${activeLikeClass}`} onClick={likes?.onClick}>
